	...
	while (fzsync_run_a(&pair)) {
		...
		for (now = 0; now <= fin; now++) {
			if (now >= ad.critical_s &&
			    now <= ad.critical_t && fzsync_atomic_load(&d) > 0)
				fin = ad.return_t;

			if (now >= a.critical_s &&
			    now <= a.critical_t && fzsync_atomic_load(&c) == 1) {
				fzsync_atomic_add_return(1, &c);
				critical++;
			}
